...
public class CommentFormValidator implements Validator {
	...
	public void validate(Object target, Errors errors) {
		...
		if(!new LengthValidator(3, 20).isCorrect(form.getName())){
			errors.rejectValue("name", "length");
		}
		if(!new LengthValidator(3, 20).isCorrect(form.getPhone())){
			errors.rejectValue("phone", "length");
		}
		if(!new LengthValidator(3, 20).isCorrect(form.getEmail())){
